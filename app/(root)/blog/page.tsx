...
const Blog = () => {
  ...
  const [allBlog, setBlog] = useState<any[]>([
    { image: "", _id: "", title: "", content: "", email: "", user: "" },
  ]);
  //   const data = {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/api/getBlog")
      .then((res) => setBlog(res.data.blog))
      .catch((e) => console.log(e));
  }, []);
  console.log(allBlog);
  return (
    <div className="flex min-h-[100vh] w-[100vw] flex-col p-28  bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))]  from-blue-200 to-blue-400">
      ...
      <div className="lg:text-5xl md:text-4xl text-lg font-extrabold flex flex-row justify-center ">
        Community Forum
      </div>
      <div className=" flex flex-col gap-4 p-14">
        {allBlog && allBlog.map((data, i) => <Blogcard data={data} key={i} />)}
      </div>
    </div>
  );
};
